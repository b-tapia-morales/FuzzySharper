...
public class CertaintyComparer(IWorkingMemory memory, IOperatorFamily family) : IRuleComparer
{
    ...
    {
        var (cf1, firstIsPresent) = (x.CertaintyFactor, x.EvaluatePremiseWeight(Memory, Family).IsSome(out var w1));
        var (cf2, secondIsPresent) = (y.CertaintyFactor, y.EvaluatePremiseWeight(Memory, Family).IsSome(out var w2));
